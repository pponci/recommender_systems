...
def load_model_and_data():
    ...
    user_item_matrix_train = sparse.load_npz(
        "../data/processed/user_item_matrix_train.npz"
    )
    user_item_matrix_test = sparse.load_npz(
        "../data/processed/user_item_matrix_test.npz"
    )

    return model, mappings, user_item_matrix_train, user_item_matrix_test


def ranking_metrics_at_k(
    model, train_user_items, test_user_items, K=10, show_progress=True
):
    """
    ...
    while start_idx < len(users_with_test_data):
        ...
        recommended_items, _ = model.recommend(
            batch_users, train_user_items[batch_users], N=K
        )
        start_idx += batch_size

        for user_idx, user_id in enumerate(batch_users):
            test_items = set(
                test_user_items.indices[
                    test_user_items.indptr[user_id] : test_user_items.indptr[
                        user_id + 1
                    ]
                ]
            )
    ...
    return {"precision": precision, "map": mean_ap, "ndcg": mean_ndcg, "auc": mean_auc}


def evaluation_pipeline():
    model, mappings, user_item_matrix_train, user_item_matrix_test = (
        load_model_and_data()
    )

    eval_results = ranking_metrics_at_k(
        model=model,
        train_user_items=user_item_matrix_train,
        test_user_items=user_item_matrix_test,
    )
